...
module.exports = {
  ...
  userById: (id, callback) => {
    pool.query(
      `SELECT registration.user_id,user_email,first_name,last_name FROM registration LEFT JOIN profile ON registration.user_id = profile.user_id WHERE registration.user_id = ?`,
      [id],
      (err, result) => {
        if (err) {
          return callback(err);
